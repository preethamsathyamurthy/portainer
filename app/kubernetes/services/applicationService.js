    ...
    if (services) {
      ...
      services.forEach(async (service) => {
        ...
        if (service.Ingress) {
          ...
          /**
           * Each service which has ingress attached to it will have a new object ( ingresses ), this object is
           * base on formValues, because formValues will be always the same for this services loop, therefore
           * new ingress object always start from whatever existing ingress, we need to pick up the correct ingress
           * generated by this function: [KubernetesIngressConverter.newApplicationFormValuesToIngresses], then
           * join them into new array: [currentIngress], then at the end, generate new complete ingress object
           * to pass into [_generateIngressPatchPromises] function.
           *  */
